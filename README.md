...
## 12. Deploying to GitHub

This project is ready to push to a public/private GitHub repo. The
`.gitignore` already excludes runtime data, secrets, and IDE files.

### One-time setup

1. **Create a new empty repo on GitHub**
   - Go to https://github.com/new
   - Repository name: `cricket-etl-pipeline` (or your choice)
   - Description: `End-to-end local data pipeline: TheSportsDB → MinIO → DuckDB → dbt → Streamlit`
   - Visibility: **Public** (recommended for portfolio) or Private
   - **Do NOT** initialize with README, .gitignore, or license — we already have those
   - Click **Create repository**
   - Copy the repo URL (e.g. `https://github.com/<your-username>/cricket-etl-pipeline.git`)
...
2. **Initialize the local repo and make the first commit**
   ...
   git branch -M main
   git add .
   git status                  # REVIEW: confirm no .env, no docker/minio_data, no logs are staged
   git commit -m "Initial commit: cricket ETL pipeline (TheSportsDB -> MinIO -> DuckDB -> dbt -> Streamlit)"
   ```

3. **Push to GitHub**

   ```powershell
   git remote add origin https://github.com/<your-username>/cricket-etl-pipeline.git
   git push -u origin main
   ```

   > If you get `error: failed to push some refs`, the GitHub repo had
   > something committed (e.g. you ticked "Add README"). Run
   > `git pull origin main --rebase` first, then push again.
...
git add .
git status
git commit -m "Add T20 Blast league to CRICKET_LEAGUES"
git push
```

### (Optional) Add a great README header image

The `screenshots/architecture.png` reference at the top of the README will
look like a broken image until you push the file. To make the README look
polished before sharing:

1. Run the pipeline (`docker compose up -d` + trigger the DAG)
2. Take the screenshots listed in `screenshots/SCREENSHOTS.md`
3. Save them as PNG in `screenshots/` with the exact file names
4. `git add screenshots/ && git commit -m "Add dashboard screenshots" && git push`

### (Optional) Connect a profile repo

If you want to embed a live demo link in your portfolio:

1. Add a badge block to the top of the README:
...
2. Enable GitHub Pages on the repo (Settings → Pages → Source: `main`,
   `/` root) so the README renders with images on the public web.

### (Optional) Run CI / pre-commit hooks

A minimal `.github/workflows/lint.yml` will run `flake8` on every push.
Create the file with this content and commit it:
...
jobs:
  flake8:
    ...
    steps:
      ...
      - run: flake8 scripts/ dags/ --max-line-length=120 --ignore=E501,W503,E203,F401
```
